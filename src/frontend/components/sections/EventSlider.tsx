...
export default function EventSlider({events}: EventSliderProps) {
    ...
    return (
        <section className="relative py-16 overflow-hidden hidden md:flex">
            ...
            <div
                className="absolute top-20 left-10 w-20 h-20 bg-purple-500/10 rounded-full blur-xl animate-pulse"></div>
            <div
                ...
                <div className="relative h-[500px] rounded-3xl overflow-hidden shadow-2xl">
                    {events.map((event, index) => (
                        ...
                        >
                            ...
                            <div
                                className="absolute inset-0 bg-gradient-to-r from-black/90 via-black/60 to-black/30 z-10"></div>
                        ...
                        {formatDate(event.date)} | {formatTime(event.time)}
                                        ...
                                        <div className="mt-8">
                                            <Button isHref={true} href={event.detail}
                                                    className="bg-purple-600 hover:bg-purple-700 text-white rounded-full px-8 py-6 shadow-lg">
                                                Подробнее
                                            </Button>
                                        </div>
                                    </div>
                    ...
                    <div
                        className="absolute bottom-6 left-1/2 transform -translate-x-1/2 z-30 flex space-x-2 bg-black/30 backdrop-blur-sm rounded-full px-4 py-2">
                        {events.map((_, index) => (
                            <button
                                key={index}
